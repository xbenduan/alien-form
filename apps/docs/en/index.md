...
- Field-owned property-level `reactions` with exactly four rule types: `static`, `expression`, `match`, and `computed`.
...
- Enterprise auditability: async requests are owned by application-level `reactionHandlers`; core does not fetch URLs.
