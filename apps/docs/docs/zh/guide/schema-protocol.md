...
`state` 不是一个独立的小系统，它只是字段初始状态的快捷写法。
...
- `visible/hidden/display` 最终统一为 `display`
- `readOnly/readPretty/disabled/editable/pattern` 最终统一为 `pattern`
...
`enum` 和 `dataSource` 最终都会收敛为字段的 `dataSource`。
