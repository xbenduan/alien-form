...
AlienForm 协议借鉴 Formily 的 Schema Form 思想，但不追求兼容。企业版协议强调可审计、可治理和低心智负担。
...
双层协议会把“条件、分支、动作、目标字段”混在一起，难以审计。AlienForm 选择主协议单层化：`x-reaction[key] = rule`，key 就是被派生的字段属性。
