...
recordRoutes.post("/list", async (c) => {
  ...
  return ok(c, await c.get("core").records.list(body, c.get("session").userId));
});
...
recordRoutes.post("/options", async (c) => {
  ...
  return ok(c, await c.get("core").records.options(body, c.get("session").userId));
});
...
recordRoutes.post("/subtree", async (c) => {
  ...
  return ok(c, await c.get("core").records.subtree(body, c.get("session").userId));
});
...
recordRoutes.post("/:model/actions/:command", async (c) => {
  ...
  return ok(
    c,
    await c.get("core").records.executeCommand(model, command, input, c.get("session").userId),
  );
});
...
recordRoutes.get("/:model/:id", async (c) => {
  ...
  return ok(c, await c.get("core").records.get(model, id, c.get("session").userId));
});
...
recordRoutes.post("/:model", async (c) => {
  ...
  const record = await c
    .get("core")
    .records.create(c.req.param("model"), values, c.get("session").userId);
...
recordRoutes.put("/:model/:id", async (c) => {
  ...
  const value = await c.req.json();
  const values = parseInput(() => recordValuesSchema.parse(value));
  return ok(c, await c.get("core").records.update(model, id, values, c.get("session").userId));
});
...
recordRoutes.post("/:model/batch-delete", async (c) => {
  ...
  await c.get("core").records.removeMany(c.req.param("model"), ids, c.get("session").userId);
  return ok(c, null);
});
...
recordRoutes.delete("/:model/:id", async (c) => {
  const { model, id } = c.req.param();
  await c.get("core").records.remove(model, id, c.get("session").userId);
  return ok(c, null);
});
