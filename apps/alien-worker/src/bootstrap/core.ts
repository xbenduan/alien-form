...
export interface WorkerCoreInput {
  ...
  /** 模型定义注册表；测试可以注入最小定义集合。 */
  modules?: ModelModules;
}

/** AlienBase 在当前 Worker 项目中的唯一组合根。 */
export const createCore = defineCore(() => {
  const categoryModelCode = "_sys_model_category";
  const modelLoaders = {
    [categoryModelCode]: () => import("../models/_sys_model_category/index.ts"),
    _sys_role: () => import("../models/_sys_role/index.ts"),
    _sys_user: () => import("../models/_sys_user/index.ts"),
  } as const;
  const modelModules = new ModelModules(modelLoaders);

  return {
    models: modelLoaders,
    create({ db, modules = modelModules }: WorkerCoreInput) {
      ...
      const access = new AccessControl(profiles, {
        publicModelNames: new Set([categoryModelCode]),
      });
      const models = new ModelService(
        ...
        access,
        compiledModels,
        new SystemModelGroupPolicy(compiledModels, recordRepository, categoryModelCode),
      );
