...
export function useRecordPage(modelName: string, options: UseRecordPageOptions): UseRecordPageResult {
  ...
  return {
    ...
    submitAdd: async (values) => {
      const success = await store?.submitAdd(values);
      if (success && store?.actionOpenMode() === 'page') {
        onRouteActionChange({ mode: 'closed' });
      }
    },
    submitEdit: async (values) => {
      const success = await store?.submitEdit(values);
      if (success && store?.actionOpenMode() === 'page') {
        onRouteActionChange({ mode: 'closed' });
      }
    },
