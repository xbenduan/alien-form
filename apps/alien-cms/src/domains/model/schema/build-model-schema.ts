...
function buildFieldSchema(
  ...
  const dataSource = parseJsonText(
    draftField.dataSourceText,
    `${draftField.title || draftField.key} dataSource`,
  );
  const defaultValue = parseJsonText(
    draftField.defaultValueText,
    `${draftField.title || draftField.key} default`,
  );

  const validReactions = draftField.reactions.filter((reaction) =>
    reaction.mode === "expression"
      ? Boolean(reaction.expressionText.trim())
      : Boolean(reaction.handler),
  );
  const xReaction =
  ...
  const reactionConfigs = validReactions.reduce<Record<string, Record<string, unknown>>>(
    (configs, reaction) => {
      if (reaction.mode !== "handler") return configs;
      const params = Object.fromEntries(
        Object.entries(reaction.handlerParams).filter(([, value]) => value.trim()),
      );
      if (Object.keys(params).length > 0) configs[reaction.target] = params;
      return configs;
    },
    {},
  );
  ...
  const isObject = draftField.type === "object";
  const isLayout = draftField.type === "layout";
  const isObjectArray = draftField.type === "array";
  const fieldType: CmsFieldSchema["type"] =
    draftField.type === "layout" ? undefined : draftField.type;
  const baseTableMeta = {
  ...
  if (isLayout) {
    ...
    return {
      ...layoutSchema,
      "x-layout": draftField.component || "SectionCard",
  ...
  if (isObjectArray) {
    return {
      ...
      items: {
        type: "object",
        title: draftField.itemTitle || "Item",
        properties: buildNestedProperties(children, pathLabel),
      },
    };
  }
...
export function buildModelSchema(draft: ModelBuilderDraft): CmsModelSchema {
  ...
  return {
    ...
    "x-model": {
      ...
      table:
        draft.tableVisibleFields.length > 0 || draft.tableDefaultWidth != null
          ? {
