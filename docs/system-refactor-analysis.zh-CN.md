...
| 运行时能力访问 | 注册能力统一按完整 code 访问：`$service(code)(params)` / `$utils(code)()` / `$enum(code)`；查询与表单值仍使用 `$query.xxx` / `$values.xxx` |
...
- `FormConfig.{schema, definitions, scope, initialValues, onError}` —— `scope` 是注入 `$service/$utils/$enum/$query` 运行时能力的天然缝（**注意：这不是被删掉的那个跨组件通信频道 scope**，见下 §改动5）。
...
4. **收紧 `buildExpressionScope`：不再把 values/兄弟字段扁平上浮到顶层**。现状 [buildExpressionScope:894-908](file:///Users/bytedance/Documents/cowork/alien-form/packages/core/src/form.ts#L894-L908) 把 `...values` 与兄弟/父级子字段 spread 成顶层裸标识符（Formily 式 `{{ fieldName }}`）。新协议一律 `{{ $values.x }}` 读；保留扁平上浮会与 `$service`/`$utils` 命名冲突，且 `new Function` body 无法安全解构开放标识符集。改成**显式闭合形状**：`{ $values, $self, $form, $value, $row, $path, $service, $utils, $enum, $query }`，去掉顶层 `...values`/兄弟 spread，使 `new Function` 解构列表稳定。
5. **`config.scope` 保留为注入缝，明确其语义**。本轮删的是"跨组件通信频道 scope"；core 的 `FormConfig.scope` 是另一回事——engine（或 core 的独立使用者）用它注入 `$service/$utils/$enum/$query`。保留，但在类型/文档上把两者区分清楚，避免混淆。
...
│   ├── scope.ts           #   作用域装配：$service/$utils/$enum/$query/$values
...
| 业务数据 | props 里 `{{ $service("code") }}` / `{{ $utils("code") }}`，返回注册值；服务函数可由组件调用，也可写成 `{{ $service("code")(params) }}` |
...
| 运行时能力 | 注册值统一使用完整 code accessor：`$service(code)` / `$utils(code)` / `$enum(code)`；`$query` / `$values` 保持对象访问 |
...
function compileExpr(raw: string): (scope: Scope) => unknown {
  ...
  const fn = new Function("$scope", `
    const { $service, $utils, $enum, $query, $values } = $scope;
    return ${src};
  `);
...
- 隐式 `()=>expr`：值表达式 `{{ $enum("status") }}` 编译成返回该值的函数；含 `=>` 的原样成函数工厂。
...
路由匹配 CompiledRoute
  ...
  → createForm({
       ...
       scope:       { $service, $utils, $enum, $query },  // 注入运行时能力（无 handlers）
     })
  → form.mount()
```
...
- `$service(code)` → 返回 registry 里的服务函数；`$service(code)(params)` 内联调用。
- `$utils(code)` → 返回 registry 里的工具值。
- `$enum(code)` → 返回 registry 里的枚举值。
...
- **注册 code 是不透明字符串**：`.`、`-` 等字符均不表示对象层级，统一通过 `$service(code)`、`$utils(code)`、`$enum(code)` 精确查询。求值器必须放开函数调用。
