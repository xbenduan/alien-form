...
const components = {
  Input: (props: any) => {
    return <input data-testid={props['data-testid'] || 'input'} value={props.value ?? ''} onChange={e => props.onChange(e.target.value)} />
  },
  ArrayView: (props: any) => {
    return (
      <div data-testid="array-view">
        <div data-testid="rows-count">{props.rows?.length || 0}</div>
        <button type="button" data-testid="add" onClick={() => props.onAdd()}>Add</button>
      </div>
    )
  }
}
