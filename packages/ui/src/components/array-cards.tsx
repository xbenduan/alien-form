    ...
    return (
      <div ref={ref} className={cn("space-y-3", className)}>
        {rows.map((rowFields, index) => (
          <div key={index} className="relative rounded-lg border border-border/50 bg-card p-4 shadow-sm">
              ...
              {!disabled && !readOnly && (
                <div className="flex items-center gap-1">
                  <button
                    ...
                    disabled={index === 0}
                    className="p-1 rounded hover:bg-accent disabled:opacity-30 disabled:cursor-not-allowed text-muted-foreground"
                    title="Move up"
                  >
        ...
        {canAdd && (
          <button
            type="button"
            onClick={() => onAdd?.()}
            className="w-full py-2 rounded-lg border border-dashed border-border/60 text-sm text-muted-foreground hover:border-primary/60 hover:text-primary transition-colors"
          >
            {addText}
          </button>
        )}
