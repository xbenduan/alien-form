...
import type { UiDefinition } from "../core/registry";
...
export function RenderNode({
  ...
  const definition = runtime.registry.ui.resolve(node.component, page.domain) as
    | UiDefinition<React.ComponentType<ComponentProps>>
    | undefined;
  ...
  const wrapped = (
    <NodeContext.Provider value={{ slots: node.slots }}>{element}</NodeContext.Provider>
  );
...
export function RenderChildren({ children }: { children: UiNode[] }): React.ReactNode {
  return children.map((child, i) => <RenderNode key={`${child.component}-${i}`} node={child} />);
}

export function Slot({ name, children }: { name: string; children?: React.ReactNode }) {
  const nodeCtx = React.useContext(NodeContext);
  const slotNode = (nodeCtx?.slots as Record<string, UiNode> | undefined)?.[name];
  return slotNode ? <RenderNode node={slotNode} /> : <>{children}</>;
}
