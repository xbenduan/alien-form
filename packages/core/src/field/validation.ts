...
import type { FieldError, IField, IFieldSchema, Validator, ValidatorFn, ValidatorRule } from '../types'
...
export async function runValidatorRule(
  ...
  if (rule.exclusiveMinimum !== undefined && typeof value === 'number' && value <= rule.exclusiveMinimum) {
    errors.push({ message: rule.message || `Must be > ${rule.exclusiveMinimum}`, type: 'exclusiveMinimum' })
  }
  if (rule.exclusiveMaximum !== undefined && typeof value === 'number' && value >= rule.exclusiveMaximum) {
    errors.push({ message: rule.message || `Must be < ${rule.exclusiveMaximum}`, type: 'exclusiveMaximum' })
  }
  if (rule.multipleOf !== undefined && typeof value === 'number' && value % rule.multipleOf !== 0) {
    errors.push({ message: rule.message || `Must be a multiple of ${rule.multipleOf}`, type: 'multipleOf' })
  }
...
function validateFormat(format: string, value: any, message?: string): FieldError | null {
  ...
  switch (format) {
    case 'email':
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(str)) return { message: message || 'Invalid email', type: 'format' }
      break
    case 'url':
      if (!/^https?:\/\/.+/.test(str)) return { message: message || 'Invalid URL', type: 'format' }
      break
    case 'phone':
      if (!/^[\d\s\-+()]+$/.test(str)) return { message: message || 'Invalid phone number', type: 'format' }
      break
    case 'number':
    ...
    case 'integer':
      ...
      break
    case 'idcard':
      if (!/^(\d{15}|\d{17}[\dXx])$/.test(str)) return { message: message || 'Invalid ID card number', type: 'format' }
      break
    case 'ip':
      if (!/^(\d{1,3}\.){3}\d{1,3}$/.test(str)) return { message: message || 'Invalid IP address', type: 'format' }
      break
    case 'ipv6':
      if (!/^([0-9a-fA-F]{1,4}:){7}[0-9a-fA-F]{1,4}$/.test(str) && !/^(([0-9a-fA-F]{1,4}:)*):?(([0-9a-fA-F]{1,4}:)*[0-9a-fA-F]{1,4})?$/.test(str)) {
