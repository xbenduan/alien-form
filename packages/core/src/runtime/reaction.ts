...
export function buildValueScope(host: RuleRuntimeHost, value: any, field?: IField): Record<string, any> {
  return {
    $self: field,
    $form: host.form,
...
export function applyReactionValue(
  ...
  switch (reactionKey as SchemaReactionKey | string) {
    ...
    case 'dataSource':
      ...
      break
    default:
      host.emitError({ scope: 'reaction', path: field.path, key: reactionKey, message: `unsupported reaction key "${reactionKey}"` })
  }
}
