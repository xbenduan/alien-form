...
describe("normalizeValidationErrors", () => {
  ...
  it("maps false to a generic invalid error", () => {
    expect(normalizeValidationErrors(false)).toEqual([
  ...
  it("maps an array of mixed results, skipping passing entries", () => {
    const out = normalizeValidationErrors([
      true,
      "e1",
      false,
      null,
  ...
  it("maps a single error object, defaulting its type", () => {
    expect(normalizeValidationErrors({ message: "only-msg" })).toEqual([
      { message: "only-msg", type: "x-validate" },
    ]);
  });
