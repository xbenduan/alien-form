...
describe("reaction — value target", () => {
  ...
  it("warns when value reaction targets a non-primitive field", () => {
    ...
    form.mount();
    expect(
      errors.some((e) => e.key === "value" && e.message.includes("only valid for primitive")),
    ).toBe(true);
  });
  ...
  it("ignores an undefined reaction result (no overwrite)", () => {
    ...
    const form = createForm({
      schema,
      initialValues: { a: "keep" },
      handlers: { maybe: () => undefined },
    });
    form.mount();
...
describe("reaction — rows target", () => {
  ...
  it("warns when rows reaction targets a non-array field", () => {
    ...
    form.mount();
    expect(errors.some((e) => e.key === "rows" && e.message.includes("only valid for array"))).toBe(
      true,
    );
  });
});
